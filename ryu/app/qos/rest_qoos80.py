...
#        selfovs_bridge = None
	...
	global last_queues
        ...
        self.restQos_muster = {REST_MATCH:self.rest_match , REST_ACTION:self.rest_action}
	...
	global last_queues
	#global last_selfqueue_list
	#last_selfqueue_list = selfqueue_list
        timer = threading.Timer(int(rest[REST_TIME]), self.return_last_queue)
        timer.start()
	...
	parent_dict = {"max_rate":str(parent_max_rate)}
	last_queues = []
	...
	for k in queue.keys():
		config_value = queue[k]["config"]
		...
		#if config_value == {"max-rate":str(parent_max_rate)}:
			#del queue[k]
			#continue
	        if config_value_key_list[0] == "max-rate":
	        	max_rate = config_value["max-rate"]
			new_dict = {}
		        new_dict[REST_QUEUE_MAX_RATE] = max_rate
		        #print new_dict
			last_queues.append(new_dict)
			queues.append(new_dict)
		elif config_value_key_list[0] == "min-rate":
		        min_rate = config_value["min-rate"]
		        new_dict = {}
			old_dict = {}
			old_dict[REST_QUEUE_MIN_RATE] = min_rate
			if REST_QUEUE_MIN_RATE in action_value_keys and rest.get(REST_PRIORITY,0) > 1 :
			   int_min_rate = int(parent_max_rate) - int(come_min_rate)-int(parent_max_rate)/20
			   min_rate = str(int_min_rate)
			#else:
			...
			new_dict[REST_QUEUE_MIN_RATE] = min_rate
			queues.append(new_dict)
			last_queues.append(old_dict)
			#print new_dict
		else:
			print "1"	
	
	#last_queues.append(parent_dict)
	if come_min_rate != None:
		if  rest.get(REST_PRIORITY,0) > 1 :
			int_come_min_rate = int(come_min_rate) + int(parent_max_rate)/20
		else:
			int_come_min_rate = int(come_min_rate)
		queues.append({REST_QUEUE_MIN_RATE:str(int_come_min_rate)})
		#queues.append({REST_QUEUE_MIN_RATE:come_min_rate})
	else:
		queues.append({REST_QUEUE_MAX_RATE:come_max_rate})

        new_restQueue = self.restQueue_muster
#        new_restQueue[REST_QUEUES] =  queues    # setQueue

        new_restQos_element = self.restQos_muster
	...
	msg=[queues, selfqueue_list]
       
	
	self.set_queue_forAll(new_restQueue , selfqos_list , 0)
	return REST_COMMAND_RESULT, msg 
    ...
    def set_queue_forAll(self, rest_setQueue , rest_setQos , vlan_id):
        global selfovs_bridge
        global selfqos_list
        selfqos_list = rest_setQos        
        if selfovs_bridge is None:
            msg = {'result': 'failure',
                   'details': 'ovs_bridge is not exists'}
            return REST_COMMAND_RESULT, msg
...
#            selfqos_list.append(restElement)
        ...
        for i in range(length):
            ...
            for rest_element in rest_setQos:
                ...
                self.set_qos(rest_element , 0 , None)
     
        return "chenggong"



    def return_last_queue(self):
#        global selfqueue_list
        global selfqos_list
        global timer
        #global selfqueues_single
	global last_queues
        ...
        high_priority_qos = selfqos_list[-1]
	high_priority_qos[REST_ACTION][REST_ACTION_QUEUE] = "0" 
	selfqos_list[-1] = high_priority_qos
        #selfqueues_single.pop(-1)
        new_restQueue = self.restQueue_muster
        new_restQueue[REST_QUEUES] = last_queues
#        timer.cancel() 
