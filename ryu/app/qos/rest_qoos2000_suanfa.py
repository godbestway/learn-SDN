...
#        selfovs_bridge = None
	...
	outside_waiters = waiters
        #haha = []
#        first_time = True
#        mss = self.ofctl.get_flow_stats(self.dp , waiters)
        if first_time == False:
#           first_time = True 
           last_port_stats = self.get_port_stats()
           self.request_flow_stats() 
#           haha = self.port_stats_stop()
#           return mss
#           return last_port_stats
#        first_time = True
#        timer = threading.Timer(int(rest[REST_TIME]) , self.delete_last_queue)
#        timer.start()
	...
	for k in queue.keys():
		config_value = queue[k]["config"]
			...
			#if REST_QUEUE_MIN_RATE in action_value_keys and rest.get(REST_PRIORITY,0) > 0 :
...
#        new_restQueue[REST_QUEUES] =  queues    # setQueue
	...
	last_queues = queues
        developed_queues = develop_new_queues(queues)
	new_restQueue[REST_QUEUES] = developed_queues
...
#left_max_rate = 0
   # def develop_regular_queues(queue_list, come_min_rate, come_max_rate):
