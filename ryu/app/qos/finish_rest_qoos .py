...
class QoS(object):
    ...
    #add new slice according the request from the client
    @rest_command
	...
	global setting_finished
	setting_finished = False

	outside_waiters = waiters
	# when receive request for the first time,start the timer.
	if first_time == False:
           last_port_stats = self.get_port_stats()
           self.request_flow_stats() 
	
	# read & save the request content	
	...
	come_max_rate=action_value.get(REST_QUEUE_MAX_RATE , None)
	come_min_rate=action_value.get(REST_QUEUE_MIN_RATE , None)
	
	new_restQueue , regular_queues = self.develop_regular_queues(port, priority, come_min_rate, come_max_rate)
        new_regular_queues = copy.deepcopy(regular_queues)
	advanced_queues = self.develop_advanced_queues(new_regular_queues)
	new_restQueue[REST_QUEUES] = advanced_queues
	
#	msg=[port, selfqueue_list , selfqos_list]
        # set the new slice according to the request.
	self.set_queue_forAll(new_restQueue , selfqos_list , 0 , waiters)
	setting_finished = True
	return REST_COMMAND_RESULT,"put rules finished"
	

    # add new queue and qos.    
    ...
    # redistribute service(rate) for all queues according to the priority 
	...
	pri_set =set()
	pri_list =[]
	#get all priorty, from large to small sort
    ...
    # send the request of the flow table information every 30 seconds, and determine whether sclices should be deleted or not.
    def request_flow_stats(self):
	   ...
	   if setting_finished == False:
                timer = threading.Timer(10 , self.request_flow_stats)
	   else:
    ...
    # obtain the flows in byte for every slice and host.    
    ...
    # return all the slices, which did not recieve any flows in the last period.
    ...
    # delete all the slices,which should be deleted.
	...
	global original_port_rate
        ...
        for i in reply:
            first = selfqos_list[0]
            print "first" , first[REST_MATCH][REST_TP_DST]
            if i == first[REST_MATCH][REST_TP_DST]:
               reply.remove(i)    
	...
	port = []
        ...
        print "origianl1" , original_port_rate
	
	#collect all port from orginal_port_rate

        kk = original_port_rate[0].keys()
        port_list.append(kk[0]) 
	for i in range(len(original_port_rate)):
	    qos_length = len(selfqos_list)
            for j in range(qos_length):
          	port = original_port_rate[i].keys()
                p = selfqos_list[j][REST_MATCH][REST_TP_DST]
		if port[0] == p:
            	   port_list.append(p)
	print "port_list",port_list
	#collect all rate of port, which are not in reply
	# and develop a current queue
	for i in range(len(port_list)):
		if port_list[i] in reply:
			pass
		else:
			port_rate = copy.deepcopy(original_port_rate[i])
			rate = port_rate[port_list[i]]
			current_queues.append(rate)

        print "selfqos_list",selfqos_list,"reply",reply
	#delete all qos_list referenced reply
	qos_list = []
	length = len(reply)
	for i in range(length):
		for j in range(len(selfqos_list)):
	...
	#reset related queue_number
        print "current_queues" , current_queues , "original2" , original_port_rate , "selfqoslist" , selfqos_list
        for i in range(len(current_queues)-1):
            qos_element = selfqos_list[i]
            qos_element[REST_ACTION][REST_ACTION_QUEUE] = str(i+1)

        c_queues = copy.deepcopy(current_queues)
	advanced_queues = self.develop_advanced_queues(c_queues)
     
	new_restQueue = copy.deepcopy(self.restQueue_muster)
	new_restQueue[REST_QUEUES] = copy.deepcopy(advanced_queues)
	print "--------------------\n"
	print "selfqos_list:",selfqos_list
	print "-------------------\n"
